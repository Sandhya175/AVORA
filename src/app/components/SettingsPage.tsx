...
export function SettingsPage() {
  ...
  const [notifSettings, setNotifSettings] = useState({ taskReminders: true, focusAlerts: true, weeklyReport: true, aiInsights: false, teamUpdates: true });
  const [prodSettings, setProdSettings] = useState({ autoFocus: true, breakReminders: true, dailyGoal: 8, pomodoroLength: 25, shortBreak: 5, longBreak: 15 });
  const [profile, setProfile] = useState({ name: 'Sandhya Tiwari', email: 'sandhya@company.com', role: 'Product Designer', timezone: 'Asia/Kolkata' });
    ...
    switch (activeSection) {
      case 'profile':
        return (
          <div>
            {/* Avatar */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, padding: '20px', background: isDark ? 'rgba(139,92,246,0.06)' : 'rgba(139,92,246,0.04)', borderRadius: 14, border: `1px solid ${t.border}` }}>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              style={{ marginTop: 20, background: 'linear-gradient(135deg,#8B5CF6,#6366F1)', border: 'none', borderRadius: 10, padding: '11px 28px', color: 'white', cursor: 'pointer', fontSize: 14, fontWeight: 600, boxShadow: '0 4px 16px rgba(139,92,246,0.4)' }}
            >
              Save Changes
            </motion.button>
          </div>
        ...
        return (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 24 }}>
              {[
                { label: 'Dark', icon: Moon, isActive: isDark },
                { label: 'Light', icon: Sun, isActive: !isDark },
                { label: 'System', icon: Monitor, isActive: false },
              ].map(opt => (
                <div
                  key={opt.label}
                  onClick={() => (opt.label === 'Dark' && !isDark) || (opt.label === 'Light' && isDark) ? toggleTheme() : undefined}
                  style={{
                    padding: '16px', borderRadius: 12, cursor: 'pointer', textAlign: 'center',
                    border: opt.isActive ? `2px solid ${t.primary}` : `1px solid ${t.border}`,
                    background: opt.isActive ? `${t.primary}12` : (isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)'),
                    boxShadow: opt.isActive ? `0 0 16px ${t.primary}25` : 'none',
                  }}
                >
                  <opt.icon size={20} color={opt.isActive ? t.primary : t.textMuted} style={{ margin: '0 auto 6px' }} />
                  <div style={{ fontSize: 13, color: opt.isActive ? t.primary : t.textMuted, fontWeight: opt.isActive ? 600 : 400 }}>{opt.label}</div>
                </div>
              ))}
            </div>
            ...
            {settingRow('Compact Mode', 'Reduce spacing between elements', <Toggle checked={false} onChange={() => {}} />)}
            {settingRow('Animations', 'Enable micro-interactions and transitions', <Toggle checked={true} onChange={() => {}} />)}
            {settingRow('Gradient Effects', 'Show glassmorphism and glow effects', <Toggle checked={isDark} onChange={() => {}} />)}
          </div>
        );
      ...
      case 'notifications':
        return (
          <div>
            <div style={sectionLabel}>Task Notifications</div>
            {settingRow('Task Reminders', 'Get notified before task deadlines', <Toggle checked={notifSettings.taskReminders} onChange={() => setNotifSettings(s => ({ ...s, taskReminders: !s.taskReminders }))} />)}
            {settingRow('Overdue Alerts', 'Be alerted when tasks become overdue', <Toggle checked={true} onChange={() => {}} />)}
            {settingRow('Team Updates', 'Notifications about shared task changes', <Toggle checked={notifSettings.teamUpdates} onChange={() => setNotifSettings(s => ({ ...s, teamUpdates: !s.teamUpdates }))} />)}

            <div style={sectionLabel}>Focus & Productivity</div>
            {settingRow('Focus Session Alerts', 'Timer start/end notifications', <Toggle checked={notifSettings.focusAlerts} onChange={() => setNotifSettings(s => ({ ...s, focusAlerts: !s.focusAlerts }))} />)}
            {settingRow('Break Reminders', 'Reminder to take scheduled breaks', <Toggle checked={true} onChange={() => {}} />)}
            {settingRow('Streak Alerts', 'Notify about productivity streaks', <Toggle checked={true} onChange={() => {}} />)}

            <div style={sectionLabel}>Reports & Insights</div>
            {settingRow('Weekly Report', 'Receive weekly productivity summary', <Toggle checked={notifSettings.weeklyReport} onChange={() => setNotifSettings(s => ({ ...s, weeklyReport: !s.weeklyReport }))} />)}
            {settingRow('AI Insights', 'Proactive suggestions from AI', <Toggle checked={notifSettings.aiInsights} onChange={() => setNotifSettings(s => ({ ...s, aiInsights: !s.aiInsights }))} />)}
          </div>
        );
      ...
      case 'productivity':
        return (
          <div>
            <div style={sectionLabel}>Focus Settings</div>
            {settingRow('Auto Focus Mode', 'Automatically start focus when tasks begin', <Toggle checked={prodSettings.autoFocus} onChange={() => setProdSettings(s => ({ ...s, autoFocus: !s.autoFocus }))} />)}
            {settingRow('Break Reminders', 'Remind me to take breaks', <Toggle checked={prodSettings.breakReminders} onChange={() => setProdSettings(s => ({ ...s, breakReminders: !s.breakReminders }))} />)}

            <div style={sectionLabel}>Pomodoro Configuration</div>
            {[
              { label: 'Focus Duration', key: 'pomodoroLength', unit: 'min', min: 10, max: 60 },
              { label: 'Short Break', key: 'shortBreak', unit: 'min', min: 3, max: 15 },
              { label: 'Long Break', key: 'longBreak', unit: 'min', min: 10, max: 30 },
              { label: 'Daily Task Goal', key: 'dailyGoal', unit: 'tasks', min: 1, max: 30 },
            ].map(setting => (
              <div key={setting.key} style={{ padding: '14px 0', borderBottom: `1px solid ${t.borderSubtle}` }}>
                ...
                <input type="range" min={setting.min} max={setting.max}
                  ...
                  onChange={e => setProdSettings(s => ({ ...s, [setting.key]: Number(e.target.value) }))}
      ...
      case 'security':
        return (
          <div>
            ...
            <button style={{ marginTop: 8, background: 'linear-gradient(135deg,#8B5CF6,#6366F1)', border: 'none', borderRadius: 10, padding: '10px 24px', color: 'white', cursor: 'pointer', fontSize: 14, fontWeight: 600, boxShadow: '0 4px 14px rgba(139,92,246,0.4)' }}>
              Update Password
            </button>

            <div style={sectionLabel}>Two-Factor Authentication</div>
            {settingRow('Enable 2FA', 'Add an extra layer of security to your account', <Toggle checked={false} onChange={() => {}} />)}
            ...
            <div style={{ padding: '14px', background: isDark ? 'rgba(139,92,246,0.06)' : 'rgba(139,92,246,0.04)', borderRadius: 12, border: `1px solid ${t.border}` }}>
              <div style={{ fontSize: 14, color: t.text, marginBottom: 4 }}>Current Device</div>
              <div style={{ fontSize: 12, color: t.textMuted }}>Chrome · Bengaluru, India · Active now</div>
            </div>
          </div>
        );
          ...
          <div>
            ...
            <div style={{ padding: '20px', background: 'linear-gradient(135deg,rgba(139,92,246,0.1),rgba(99,102,241,0.08))', borderRadius: 14, border: `1px solid ${t.border}`, marginBottom: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: t.text }}>Pro Plan</div>
                  <div style={{ fontSize: 13, color: t.textMuted, marginTop: 2 }}>$12/month · Renews Jun 10, 2027</div>
                </div>
            ...
            {[
              { icon: Download, label: 'Export Data', sub: 'Download all your tasks and analytics', color: t.primary },
              { icon: Trash2, label: 'Delete Account', sub: 'Permanently delete your account and data', color: '#EF4444' },
            ].map(action => (
              <button key={action.label} style={{
                ...
                <ChevronRight size={14} style={{ marginLeft: 'auto', color: t.textDim }} />
              </button>
            ))}
          </div>
        );
        ...
        {sections.map(s => (
          <button
            ...
            style={{
              ...
              borderRadius: 10, cursor: 'pointer', border: 'none', marginBottom: 3, textAlign: 'left',
